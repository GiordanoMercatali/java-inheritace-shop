package org.lessons.java.Shop;

import java.math.BigDecimal;

public class Main {
    public static void main(String[] args) {
        Product apple = new Product("Apple", "Una mela al giorno toglie il medico di torno", new BigDecimal(1.22), new BigDecimal(0.22));
        // String name = apple.getName();
        // System.out.println("Product name: " + name);
        String codename = apple.getCodeName();
        System.out.println("Product: " + codename);
        String desc = apple.getDesc();
        System.out.println(desc);
        BigDecimal price = apple.getPrice();
        System.out.println("Original price: " + price + "$");
        BigDecimal taxedPrice = apple.getTaxedPrice(price);
        System.out.println("Price with taxes: " + taxedPrice + "$");
        // String code = apple.getCode();
        // System.out.println("Product code: " + code);
        
    }
}
